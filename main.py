...
async def play(interaction: discord.Interaction, url: str):
    ...
    if "entries" in playlist_info:
        ...
        asyncio.create_task(
            process_remaining_playlist(playlist_info["entries"][1:], interaction)
        )
        ...
        async with interaction.channel.typing():
            ...
            await interaction.channel.send(
                f"Song **{song_link}** has been added to the queue."
            )
...
async def process_remaining_playlist(entries, interaction):
    ...
    for entry in entries:
        try:
            ...
            await interaction.channel.send(
                f"Error occurred while processing playlist: {e}"
            )
...
async def queue_list(interaction: discord.Interaction):
    if len(queue) == 0:
        ...
        queue_titles = "\n".join(
            [f"[{song.title}]({shorten_url(song.url)})" for song in queue]
        )
